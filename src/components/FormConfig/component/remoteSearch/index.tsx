...
function Project(props: Props) {
  const { selectProps = {}, name, component = {}, formItemConfig = {}, config, forceUpdate, form } = props
  ...
  const onChange = (v, from?) => {
    ...
    if (from === 'input') {
      ...
      if (component.mode) {
        values.forEach(id => {
          listValue.push(listRef.current.find(item => item[component.optionValue] === id))
        })

      }
    }

    component?.selectedValueField?.split('.').reduce((acc, curr, index, arr) => {
            ...
            return <>
              {
                remote_from_trigger_env
                  ...
                  :
                  <Form.Item {...formItemConfig} rules={[{ required: true, message: formItemConfig.message }]} style={{ flex: 1 }}>
                    <Select
                      allowClear
                      showSearch
